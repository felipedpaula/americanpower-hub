...
export default function Show({ colaborador, resumo, anoAtual, anosDisponiveis = [], meses = [], statusOptions = [], tabs = [] }) {
    const { props } = usePage();
    ...
    const handleChangeAno = (value) => {
        router.get(`/cms/financeiro/colaboradores/${colaborador.id}`, { ano: value }, { preserveState: true, preserveScroll: true });
    };

    return (
        <CMSLayout>
            <Head title={`Financeiro - ${colaborador?.name}`} />

            <div className="space-y-6 animate-fadeIn">
                <FinanceiroTabs tabs={tabs} className="justify-start md:justify-end" />
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <Link href="/cms/financeiro/colaboradores" className="text-sm text-muted-foreground hover:text-foreground">
                            ← Voltar para listagem de colaboradores
                        </Link>
                        <h1 className="mt-2 text-3xl font-bold text-foreground dark:text-foreground">{colaborador?.name}</h1>
                        <p className="mt-1 text-sm text-muted-foreground">{colaborador?.email}</p>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                        <Badge variant={colaborador?.status === 'ativo' ? 'default' : 'secondary'}>{colaborador?.status_label}</Badge>
                    </div>
                </div>
                            ...
                            {meses.map((mes) => (
                                <Card key={mes.competencia} className="border border-border bg-card hover:border-primary transition-colors">
                                    ...
                                    <CardContent className="space-y-3 text-sm">
                                        ...
                                        <div className="pt-3">
                                            <Link href={`/cms/financeiro/colaboradores/${colaborador.id}/competencias/${mes.competencia}`}>
